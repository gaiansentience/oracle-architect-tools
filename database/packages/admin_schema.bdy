...
begin
  ...
  v_ddl := 'CREATE TABLE ' || p_source_table || '_' || p_suffix;
  v_ddl := v_ddl || ' AS SELECT ';
  v_ddl := v_ddl || ' * FROM ' || p_source_table;
...
begin
  ...
  v_ddl := v_ddl || ' TABLESPACE ' || p_tablespace || ' NOLOGGING';
...
begin
  ...
  for r in c loop
    ...
    move_table_partition_single(p_table, r.partition_name, p_tablespace, p_script, p_execute);

  end loop;

end move_table_partitions_all;
