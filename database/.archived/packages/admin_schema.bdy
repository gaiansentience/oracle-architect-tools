...
begin
  ...
  if p_update then
    v_privileges := v_privileges || 'UPDATE,';
  end if;
  if p_delete then
    v_privileges := v_privileges || 'DELETE';
  end if;
  v_privileges := rtrim(v_privileges,',');
  
  v_grant := 'GRANT ' || v_privileges || ' ON ' || p_owner || '.' || p_object_name || ' TO ' || p_user;
...
begin
  ...
  if p_tables then
    for r in t loop
      grant_dml_on_object(v_owner, r.object_name, p_user, p_insert, p_update, p_delete, p_script, p_execute);
    end loop;
  end if;
...
is
   v_ddl varchar2(1000);
begin
   v_ddl := 'create or replace directory ' || p_ora_dir || ' as ''' || p_path || '''';
   run_ddl(v_ddl, p_script, p_execute);
...
is
   v_ddl varchar2(100);
begin
  v_ddl := 'drop directory ' || p_ora_dir;
  run_ddl(v_ddl, p_script, p_execute);
end drop_directory;
...
is
   v_ddl varchar2(1000);
begin
   v_ddl := 'revoke all on directory ' || p_ora_dir || ' from ' || p_user;
   run_ddl(v_ddl, p_script, p_execute);
...
begin
   ...
   v_ddl := v_ddl || ' on directory ' || p_ora_dir || ' to ' || p_user;
   run_ddl(v_ddl, p_script, p_execute);
