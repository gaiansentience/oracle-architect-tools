create or replace package body app#log_util is
  ...
  (
    ...
    p_stop     in timestamp default null
  ) is
    function cut
    (
      p_val    in varchar2,
  ...
  begin
    ...
    r_entry.object_name   := nvl(cut(p_object,
                                     c_max_object_name),
                                 c_unspecified);
    r_entry.object_method := nvl(cut(p_method,
                                     c_max_object_name),
                                 c_unspecified);
    r_entry.reference_id  := nvl(p_ref_id,
                                 -999);
    r_entry.message       := cut(p_message,
                                 c_max_table_varchar);
    r_entry.created_by    := app#env_util.osuser;
    r_entry.created_date  := sysdate;


---TODO:  add timing_start and timing_stop to log table
    --r_entry.timing_start := p_start;
    --r_entry.timing_stop  := p_stop;

    /*TODO: calculate interval for timing and store it in log table */
