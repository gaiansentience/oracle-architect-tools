create or replace view dynamic_view_pipelined_source_v as
with pipelined_source as
(
    select
        value(t) as record_object
    from
        dynamic_view_pipelined.get_shapes(
            cursor(
                select o.* from dynamic_pivot_item_rowsource_v o
                )
        ) t
...
select
    ...
    ,case 
        when p.record_object is of type (dynamic_geometry_square) then 'SQUARE'
        when p.record_object is of type (dynamic_geometry_rectangle) then 'RECTANGLE'
        when p.record_object is of type (dynamic_geometry_circle) then 'CIRCLE'
        when p.record_object is of type (dynamic_geometry_polygon) then 'POLYGON'
        when p.record_object is of type (dynamic_geometry_line) then 'LINE'
        when p.record_object is of type (dynamic_geometry_triangle) then 'TRIANGLE'
        else 'GENERIC DYNAMIC_GEOMETRY'
    end as record_type
    ,treat(p.record_object as dynamic_geometry_square) as square
    ,treat(p.record_object as dynamic_geometry_rectangle) as rectangle
    ,treat(p.record_object as dynamic_geometry_circle) as circle
    ,treat(p.record_object as dynamic_geometry_polygon) as polygon
    ,treat(p.record_object as dynamic_geometry_line) as line
    ,treat(p.record_object as dynamic_geometry_triangle) as triangle
from 
    pipelined_source p
)
select
