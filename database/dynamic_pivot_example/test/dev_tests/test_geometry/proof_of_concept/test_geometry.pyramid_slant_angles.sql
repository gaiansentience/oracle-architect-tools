...
begin
  ...
  l_slant_height := test_geometry.pyramid_slant_height(
    p_base => p_base,
    p_height => p_height,
    p_sides => p_sides
  );

  l_inradius := test_geometry.polygon_inradius(p_sides=> p_sides, p_length=>p_base);
    
  l_slant_angle := test_geometry.pyramid_slant_angles(
    p_height => p_height,
