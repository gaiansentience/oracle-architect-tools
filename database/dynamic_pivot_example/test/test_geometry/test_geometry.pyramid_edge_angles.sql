...
begin
  ...
  l_edge_height := oa_geometry.pyramid_edge_height(
    p_base => p_base,
    p_height => p_height,
    p_sides => p_sides
  );

  l_circumradius := oa_geometry.polygon_circumradius(p_sides=> p_sides, p_length=>p_base);
    
  l_edge_angle := oa_geometry.pyramid_edge_angles(
    p_height => p_height,
